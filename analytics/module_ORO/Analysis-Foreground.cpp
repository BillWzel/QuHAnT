...
int QuHAnTitation(cv::Mat, std::string, std::string, std::string, bool);
...
// Initializes vector for threshold function using input arguments
	...
	R.push_back(atoi(argv[8]));
        ...
        std::vector<float> S;
        S.push_back(atoi(argv[10]));
        S.push_back(atoi(argv[11]));
        S.push_back(atoi(argv[12]));
        S.push_back(atoi(argv[13]));
        S.push_back(atoi(argv[14]));
        S.push_back(atoi(argv[15]));
...
// Test run using predetermined test image, if not predetermined result, stops program
	if (name2 == "Capture")
	{
		...
		int sum = QuHAnTitation(masked, name2, "", "", true);
...
// Goes within functions
	...
	int sum = QuHAnTitation(masked,name, proj, set, true);

        cv::Mat background_mask = threshold(mask, image, S);
        int sum2 = QuHAnTitation(background_mask, name, proj, set, false);
...
int QuHAnTitation(cv::Mat masked, std::string name, std::string proj, std::string set, bool foreground){
	...
	if (name == "Capture" and sum < 5000)
        ...
        std::string feature_str, type_str;
        if (foreground == 0) {
          feature_str = "feature";
          type_str = "";
        } else {
          feature_str = "background";
          type_str = "_background";
        }

	std::ofstream q_str;
	std::string summ_file_name = "output." + feature_str + ".full.txt";
	q_str.open(summ_file_name.c_str());

	std::ofstream s_str;
	std::string area_file_name = "output." + feature_str + ".summary.txt";
	s_str.open(area_file_name.c_str());

	//Places the quantitative information into output_full.txt
	q_str << "{\"type\": \"full"+type_str+"\", \"data\": [";
	...
	s_str << "{\"type\": \"summary"+type_str+"\", \"data\": {\"Project\":\"" << proj;
