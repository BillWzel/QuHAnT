...
# Import Tables
features.summary <- read.table('results_summary.csv', sep=',', header=TRUE)
...
#	  					  by.y=c('File','Set','Project'))
  ...
  individual.sum <- data.frame(with(annotated.summary, tapply(annotated.summary[,'Total.Area'], list(annotated.summary[,'IndividualID']),sum)))
  individual.sum <- cbind(row.names(individual.sum),individual.sum)
  colnames(individual.sum)<-c('IndividualID','Total.Area')
  individual.sum<-merge(individual.sum, metadata, by.x=c('IndividualID'),by.y=c('IndividualID'))
  individual.sum[,'Total.Area']<-as.numeric(individual.sum[,'Total.Area'])
  ...
  if((length(colnames(metadata))) >= 5) {
    for(ind in 5:length(colnames(metadata))) {
      ...
      feat.avg    <- with(individual.sum, tapply(individual.sum[,'Total.Area'], list(individual.sum[,independent]),mean))
      feat.avg[is.na(feat.avg)] <- 0
      feat.sd     <- with(individual.sum, tapply(individual.sum[,'Total.Area'], list(individual.sum[,independent]),sd))
      feat.sd[is.na(feat.sd)] <- 0
      feat.count  <- with(individual.sum, tapply(individual.sum[,'Total.Area'], list(individual.sum[,independent]),function(x) length(unique(x))))
